...
export const DoctorDashboard: React.FC = () => {
        ...
        return (
             <div className="space-y-6 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="text-sm bg-black/5 dark:bg-white/5 p-4 rounded-lg shadow-inner space-y-2">
                        <h4 className="font-semibold mb-2 text-gray-700 dark:text-gray-300">Patient Data Summary</h4>
                ...
                <div>
                    ...
                    <div className="bg-black/5 dark:bg-white/5 p-4 rounded-lg shadow-inner prose prose-sm max-w-none text-gray-600 dark:text-gray-400 max-h-40 overflow-y-auto custom-scrollbar">
                       <p className="whitespace-pre-wrap">{selectedPatient.aiSuggestions || "No suggestions available."}</p>
                    </div>
                </div>
                <div className="pt-4">
                    <button onClick={() => handleOpenChat(selectedPatient)} className="w-full bg-health-buddy-blue text-white font-bold py-4 px-6 rounded-xl shadow-md hover:animate-pulse-glow focus:outline-none focus:ring-2 focus:ring-health-buddy-blue focus:ring-opacity-75 transition-all duration-300 flex items-center justify-center transform hover:scale-[1.02] active:scale-100">
    ...
    return (
        <>
            <Head>
                <title>Doctor Dashboard | Health Buddy</title>
            </Head>
            <div className="min-h-screen flex flex-col items-center p-4 animate-fade-in">
                ...
                <main className="w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 flex-grow">
                    <div className="lg:col-span-1 bg-black/10 dark:bg-white/5 backdrop-blur-lg border border-white/20 dark:border-white/10 rounded-2xl shadow-ios-dark p-6">
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4 flex items-center"><List className="mr-3" /> Consultation Queue</h2>
                        <div className="bg-black/5 dark:bg-white/5 rounded-lg shadow-inner p-2 h-full min-h-[400px] max-h-[70vh] overflow-y-auto custom-scrollbar">
                            {consultationRequests.length > 0 ? (
                                consultationRequests.map((report) => {
                                    const { level, textColor } = getRiskDetails(report.score ?? 0);
                                    return (
                                        <button key={report.id} onClick={() => handleSelectPatient(report)} className={`w-full text-left p-3 rounded-lg flex justify-between items-center transition-all duration-200 mb-1 ${selectedPatient?.id === report.id ? 'bg-health-buddy-blue/20 ring-2 ring-health-buddy-blue shadow-glow-selected' : 'hover:bg-black/10 dark:hover:bg-white/10'}`}>
                                            <div>
                    ...
                    <div className="lg:col-span-2 bg-black/10 dark:bg-white/5 backdrop-blur-lg border border-white/20 dark:border-white/10 rounded-2xl shadow-ios-dark p-6 md:p-8">
                        {chattingWithPatient ? (
                            <ChatInterface 
                ...
                <footer className="w-full text-center mt-8 pb-4">
                 ...
                 <style>{`
                    ...
                    .dark .prose { --tw-prose-body: #9ca3af; --tw-prose-bold: #f9fafb; --tw-prose-bullets: #6b7280;}
                `}</style>
            </div>
        </>
    );
